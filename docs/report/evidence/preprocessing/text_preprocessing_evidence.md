...
After the deterministic stages, `src/text_encoder.py::encode` passes the normalised string to `sentence-transformers/all-MiniLM-L6-v2`. Its WordPiece tokenizer turns `what gate is flight xy456 leaving from` into `what gate is flight x ##y ##45 ##6 leaving from` (10 pieces); the alphanumeric code is fragmented, so identifier matching cannot be left to the embedding — it is done before this point. (Full tokenisation examples: 05.3, `data_exploration/text_preprocessing_examples.md`.)
