...
|---|---|---|
| **A — directly recorded evidence** | exact inputs, system sentences, badges, routes, scores, flags, entities, candidate lists, transcripts, KB records as captured on the deployed Space | "Passenger input"; "Actual system behaviour"; the parts of "Observed limitation" marked *Recorded* |
| **B — pre-defined expectation** | written in the working session before the scenario was run | "Expected behaviour" for scenarios 1 and 2 |
| **C — post-hoc design expectation** | reconstructed after the run from the frozen router rules (R0–R6), the retrieval cascade and the KB contents | "Expected behaviour" for scenarios 3, 4 and 5 |
| **D — analytical judgement** | the developer's reading of the recorded output | "Correct?"; the parts of "Observed limitation" marked *Interpretation* |
| **E — future-work proposal** | not implemented, not tested | "Suggested improvement" |

Sources for class A: `deployment_smoke_test.md` (summary table and observations), the screenshots
`ui_deployment_*.png`, and the verbatim page-text captures in `deployment_smoke_test_captures.md`.
Per scenario: 1 → capture A, `ui_deployment_text.png`; 2 → capture B, `ui_deployment_extra_image_only.png`;
3 → capture C, `ui_deployment_extra_voice.png`; 4 → capture D, `ui_deployment_multimodal.png`,
`ui_deployment_extra_multimodal_technical.png`; 5 → capture E, `ui_deployment_uncertainty.png`,
`ui_deployment_extra_uncertainty_technical.png`.
