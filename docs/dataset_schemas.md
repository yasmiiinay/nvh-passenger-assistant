# Dataset Schemas v1 (frozen at the foundation stage)

Schemas, populated during the development phase (counts are given in the README). Header-only manifest CSVs sit
...
| file | path relative to `data/images/`, e.g. `files/img_001.gif`; added with the vision pipeline so the manifest is runnable |
...
| split | `dev` \| `heldout` |
| file | path relative to `data/audio/`, e.g. `files/aud_001.wav` (16 kHz mono WAV); added with the speech pipeline |
